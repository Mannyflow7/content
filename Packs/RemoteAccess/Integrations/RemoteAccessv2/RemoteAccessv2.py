...
def perform_copy_command(
    ...
    This helper function was separated from command functions mainly for
    ...
    Args:
        ...
        destination_path (str):
        - Copy to remote machine from Cortex XSOAR - the remote machine file path
        to contain the copied data.
        - Copy to Cortex XSOAR from remote machine - Temp file name to save the file,
    ...
    try:
        with SCPClient(
            ssh_client.get_transport(), socket_timeout=socket_timeout
        ) as scp_client:
            if copy_to_remote:
        ...
        if "Not a directory" in str(e):
            raise DemistoException(
                f"Given destination path: {destination_path} does not exist"
                f" in remote machine.\n"
...
def perform_sftp_copy_command(
    ...
    try:
        with client.open_sftp() as sftp_client:
            if copy_to_remote:
                sftp_client.put(file_path, destination_path)
                return ""
            else:
                with tempfile.TemporaryDirectory() as temp_dir:
                    sftp_client.get(file_path, f"{temp_dir}/{destination_path}")
                    with open(f"{temp_dir}/{destination_path}", "rb") as f:
                        return f.read()
...
def create_paramiko_client(
    ...
    Args:
        ...
        timeout (int): timeout of ssh client
        private_key (str): The SSH certificate (should be PEM file based
        certificate only).
    Returns:
        (SSHClient): Paramiko client if connection was successful, exception otherwise.
    """
    if ciphers:
        # Getting available ciphers from server, in order to print an appropriate error message upon no cipher match. # noqa: E501
    ...
    try:
        ...
        if private_key:
            # authenticating with private key only works for certificates which are
            # based on PEM files.
            # (RSA private keys)
            rsa_private_key = paramiko.RSAKey.from_private_key(StringIO(private_key))  # type: ignore # [assignment] # noqa: E501
        client.connect(
            hostname=host_name,
            username=user_name,
            password=password,
...
def execute_shell_command(client: SSHClient, args: Dict[str, Any]) -> CommandResults:
    ...
    Args:
        client (SSHClient): SSH client to perform the command with.
    ...
    # exec_command returns a tuple of stdin, stdout, stderr. No need to parse stdin because it does not contain data. # noqa: E501
    # expected to have shell access, ignoring bandit error
    _, stdout, std_err = client.exec_command(command, timeout=timeout)  # nosec
    ...
    if stdout_str or std_error_str:
        ...
        readable_output = tableToMarkdown(
            f"Command {command} Outputs", outputs, removeNull=True
        )
    else:
        outputs = None
        readable_output = (
            f"### Command {command} was executed successfully without any outputs."
        )
...
def copy_to_command(
    client: SSHClient, args: Dict[str, Any], use_sftp: bool = False
) -> CommandResults:
    """
    ...
    Args:
        client (SSHClient): SSH client to perform the command with.
        args (Dict[str, Any]): Cortex XSOAR arguments.
        use_sftp (bool): use sftp instead of SCP

    Returns:
        (CommandResults).
    """

    dest_dir_arg = args.get("dest_dir", "")
    destination_path_arg = args.get("destination_path", "")
    if dest_dir_arg and destination_path_arg:
        raise DemistoException(
            'Please provide at most one of "dest_dir" argument'
            ' or "destination_path", not both.'
        )
    ...
    if not entry_id:
        raise DemistoException(
            'No entry ID path given. Please provide one of the "entry_id" (recommended)'
            ' or "entry" inputs.'
        )
    ...
    if not (file_path := file_path_data.get("path", "")):
        raise DemistoException(
            "Could not find given entry ID path. "
            "Please assure given entry ID is correct."
        )
    ...
    if destination_dir:
        execute_shell_command(client, args={"cmd": f"mkdir -p {destination_dir}"})
    ...
    if not use_sftp:
        ...
        perform_sftp_copy_command(
            client, file_path, destination_path, copy_to_remote=True
        )
    return CommandResults(
        readable_output=f"### The file corresponding to entry ID: {entry_id}"
        f" was copied to remote host."
    )


def copy_from_command(
    client: SSHClient, args: Dict[str, Any], use_sftp: bool = False
) -> Dict:
    """
    ...
    if use_sftp:
        remote_file_data = perform_sftp_copy_command(
            client, file_path, file_name, copy_to_remote=False
        )
    else:
        remote_file_data = perform_copy_command(
            client, file_path, file_name, copy_to_remote=False, socket_timeout=timeout
        )
...
def main() -> None:
    ...
    if password_param := params.get("additional_password", {}).get("password"):
        if command != "test-module" and password_param != args.get(
            "additional_password"
        ):
            raise DemistoException(
                "Additional password to use the module have been supplied.\n"
                'Please supply "additional_password" argument '
                'that matches the "Additional Password" parameter value.'
            )
    client = None
    try:
        if timeout := args.get("timeout"):
        ...
        use_sftp = argToBoolean(args.get("use_sftp", False))
        demisto.debug(f"use sftp: {use_sftp}")

        client = create_paramiko_client(
            host_name, user, password, ciphers, key_algorithms, timeout, certificate
        )
        ...
        if command == "test-module":
            ...
            return_results(copy_to_command(client, args, use_sftp))
        elif command == "copy-from":
            return_results(copy_from_command(client, args, use_sftp))
        else:
